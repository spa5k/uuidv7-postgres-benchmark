...
class DataExporter:
    ...
    def export_complete_dataset(self, results: Dict[str, Any]) -> Dict[str, str]:
        ...
        try:
            ...
            logger.info(f"Data export completed successfully. Files saved to: {self.output_base_dir}")
    ...
    def _export_performance_summary(self, results: Dict[str, Any]) -> Path:
        ...
        filename = FILE_TEMPLATES['performance_summary'].format(timestamp=self.timestamp)
    ...
    def _export_chart_data(self, results: Dict[str, Any]) -> Path:
        ...
        if chart_data['bar_chart_single_thread']:
            max_time = max(item['avg_time_us'] for item in chart_data['bar_chart_single_thread'])
            max_throughput = max(item['throughput_ops_sec'] for item in chart_data['bar_chart_throughput'])
            ...
            for item in chart_data['bar_chart_single_thread']:
                ...
                throughput_item = next(
                    (t for t in chart_data['bar_chart_throughput'] 
                ...
                if throughput_item:
                    chart_data['radar_chart_comparison'].append({
                        'function': func_name,
                        'database': db_name,
                        'speed_score': 100 * (1 - item['avg_time_us'] / max_time),  # Higher is better
                        'throughput_score': 100 * throughput_item['throughput_ops_sec'] / max_throughput,
                        'consistency_score': 100 * max(0, 1 - next(
                            (c['coefficient_of_variation'] for c in chart_data['line_chart_consistency']
                             if c['function'] == func_name and c['database'] == db_name), 0
                        ))
                    })
    ...
    def _generate_charts(self, results: Dict[str, Any]) -> Path:
        ...
        fig.suptitle('PostgreSQL UUIDv7 Benchmark Results - Professional Analysis', fontsize=20, fontweight='bold')
        
        # 1. Single-thread performance comparison
        ax1 = axes[0, 0]
        sns.barplot(data=df, x='Function', y='Avg Time (μs)', hue='PostgreSQL', ax=ax1)
        ax1.set_title('Single-Thread Performance (Lower is Better)', fontsize=14, fontweight='bold')
        ...
        ax2 = axes[0, 1]
        sns.barplot(data=df, x='Function', y='Throughput (ops/sec)', hue='PostgreSQL', ax=ax2)
        ax2.set_title('Concurrent Throughput (Higher is Better)', fontsize=14, fontweight='bold')
        ...
        ax3 = axes[1, 0]
        scatter = ax3.scatter(df['Avg Time (μs)'], df['Throughput (ops/sec)'], 
                             c=df['Consistency (CV)'], s=100, alpha=0.7, cmap='viridis')
        ax3.set_title('Performance vs Throughput (Consistency as Color)', fontsize=14, fontweight='bold')
        ...
        for func in common_functions:
            ...
            time_improvement = ((pg17_row['Avg Time (μs)'] - pg18_row['Avg Time (μs)']) / 
                               pg17_row['Avg Time (μs)'] * 100)
            throughput_improvement = ((pg18_row['Throughput (ops/sec)'] - pg17_row['Throughput (ops/sec)']) / 
                                    pg17_row['Throughput (ops/sec)'] * 100)
        ...
        if table_data:
            table = ax4.table(cellText=table_data,
                             colLabels=['Function', 'PG17 Time(μs)', 'PG18 Time(μs)', 'Time Δ', 'Throughput Δ'],
                             cellLoc='center',
                             loc='center')
            table.auto_set_font_size(False)
            table.set_fontsize(10)
            table.scale(1.2, 1.5)
            ax4.set_title('PostgreSQL 17 vs 18 Comparison', fontsize=14, fontweight='bold')
...
"""
        ...
        for result in sorted_results:
            ...
            report += f"| {result['function']} | {result['db']} | {result['pg_version']} | "
...
"""
        ...
        sorted_by_throughput = sorted(all_results, key=lambda x: x['throughput'], reverse=True)
...
"""
            ...
            for func_name in sorted(common_functions):
                ...
                time_improvement = ((pg17_data['avg_time_us'] - pg18_data['avg_time_us']) / 
                                   pg17_data['avg_time_us'] * 100)
                throughput_improvement = ((pg18_data['throughput'] - pg17_data['throughput']) / 
                                        pg17_data['throughput'] * 100)
