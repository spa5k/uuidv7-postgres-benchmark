...
class DatabaseManager:
    ...
    def check_database_info(self, config: DatabaseConfig) -> DatabaseInfo:
        ...
        with self.get_connection(config) as conn:
            with conn.cursor() as cur:
                ...
                cur.execute("SELECT version(), current_setting('server_version_num')::INTEGER")
                ...
                if version_number >= 180000:
                    try:
                        ...
                        logger.info(f"Native uuidv7() support confirmed for PostgreSQL {major_version}")
                    except Exception:
                        logger.warning(f"PostgreSQL {major_version} detected but native uuidv7() not available")
                
                # Test which functions are available
                supported_functions = self._test_function_availability(cur, major_version)
    ...
    def initialize_all_databases(self) -> Dict[str, DatabaseInfo]:
        ...
        for db_name, config in DATABASE_CONFIGS.items():
            try:
                ...
                logger.info(f"{db_name}: PostgreSQL {db_info.major_version} "
                ...
                for func_name in db_info.supported_functions:
                    ...
                    try:
                        ...
                        validation = {
                            ...
                            'all_unique': len(set(str(id) for id in test_ids)) == len(test_ids),
                            'correct_format': self._validate_id_format(test_ids[0], func_config),
                            'time_ordered': self._validate_time_ordering(test_ids, func_config) if func_config['time_ordered'] else None,
                            'sample_id': str(test_ids[0]),
                            'length_consistent': all(len(str(id)) == len(str(test_ids[0])) for id in test_ids[:10])
                        }
    ...
    def _validate_time_ordering(self, ids: List[Any], func_config: Dict[str, Any]) -> bool:
