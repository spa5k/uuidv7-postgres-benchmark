...
class BenchmarkEngine:
    ...
    def run_complete_benchmark(
        ...
        logger.info(
            f"Configuration: {self.config.benchmark_runs} runs, "
        ...
        for db_name, config in db_configs.items():
            ...
            logger.info(f"\n{'='*60}")
            logger.info(
                f"Benchmarking {db_name} (PostgreSQL {db_info[db_name].major_version})"
            )
            logger.info(f"{'='*60}")
            ...
            logger.info(
                f"Function validation completed: {len(validation_results)} functions tested"
            )

            # Run benchmarks for all supported functions
            for func_name in db_info[db_name].supported_functions:
                self._run_function_benchmark(
                    db_name, config, func_name, db_info[db_name]
                )
    ...
    def _run_function_benchmark(
        self, db_name: str, config: DatabaseConfig, func_name: str, db_info: Any
    ):
        ...
        for run_num in range(1, self.config.benchmark_runs + 1):
            ...
            try:
                ...
                # Log run summary
                logger.info(
                    f"    Single-thread: {result.single_thread_avg_ns/1000:.1f}μs avg"
                )
                logger.info(
                    f"    Concurrent: {result.concurrent_throughput_per_second:,.0f} ops/sec"
                )
    ...
    def _execute_single_benchmark_run(
        ...
        with self.db_manager.get_connection(config) as conn:
            with conn.cursor() as cur:
                # Warmup phase
                logger.debug(
                    f"    Warming up ({self.config.warmup_iterations:,} iterations)..."
                )
                self._warmup_function(cur, func_name, self.config.warmup_iterations)

                # Single-threaded benchmark
                logger.debug(
                    f"    Single-threaded ({self.config.single_thread_iterations:,} iterations)..."
                )
                ...
                logger.debug(
                    f"    Concurrent ({self.config.concurrent_workers} workers)..."
                )
    ...
    def _benchmark_single_threaded(
        self, cursor, func_name: str, iterations: int
    ) -> Dict[str, Any]:
    ...
    def _benchmark_concurrent(
        ...
        return {
            ...
            "std_dev_ns": statistics.stdev(all_times_ns)
            if len(all_times_ns) > 1
            else 0,
    ...
    def _aggregate_all_results(self):
        ...
        for db_name, results in self.results.items():
            ...
            for func_name, func_results in function_groups.items():
                ...
                all_throughputs = [
                    r.concurrent_throughput_per_second for r in func_results
                ]
                all_concurrent_avgs = [r.concurrent_avg_ns for r in func_results]

                # Calculate consistency metrics
                cv = (
                    statistics.stdev(all_single_avgs) / statistics.mean(all_single_avgs)
                    if len(all_single_avgs) > 1
                    else 0
                )
                ...
                aggregated = AggregatedResults(
                    ...
                    std_dev_single_thread_ns=statistics.stdev(all_single_avgs)
                    if len(all_single_avgs) > 1
                    else 0,
                    ...
                    std_dev_throughput=statistics.stdev(all_throughputs)
                    if len(all_throughputs) > 1
                    else 0,
                    # Consistency metrics
                    coefficient_of_variation=cv,
                    run_to_run_variance=statistics.variance(all_single_avgs)
                    if len(all_single_avgs) > 1
                    else 0,
                )
    ...
    def _prepare_final_results(self, total_time: float) -> Dict[str, Any]:
        ...
        return {
            ...
            "raw_results": {
                db_name: [asdict(r) for r in results]
                for db_name, results in self.results.items()
            },
            "aggregated_results": {
                key: asdict(result) for key, result in self.aggregated_results.items()
            },
            "database_info": {
                db_name: asdict(info)
                for db_name, info in self.db_manager.db_info.items()
            },
        }
