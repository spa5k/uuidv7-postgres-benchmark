...
from src.config import DATABASE_CONFIGS, DEFAULT_BENCHMARK_CONFIG, validate_config, load_env_overrides
...
def setup_logging(verbose: bool = False):
    ...
    # File handler
    log_file = Path('results') / 'logs' / f'benchmark_{time.strftime("%Y%m%d_%H%M%S")}.log'
...
def validate_environment():
    ...
    for db_name, config in DATABASE_CONFIGS.items():
        try:
            ...
            logger.error("Please ensure Docker containers are running: docker-compose up -d")
            return False
...
Environment Variables:
  ...
  BENCHMARK_EXTENSIVE_MODE=1                       # Enable extensive mode
    ...
    parser.add_argument('--quick', action='store_true',
                       help='Quick mode (5K iterations, 3 runs)')
    ...
    logger.info(f"  Concurrent Iterations per Worker: {config.concurrent_iterations_per_worker:,}")
    ...
    try:
        ...
        if results['aggregated_results']:
            ...
            print(f"  🚀 Fastest Single-Thread: {fastest['name']} on PG{fastest['pg_version']} ({fastest['avg_time_us']:.1f}μs)")
            print(f"  ⚡ Highest Throughput: {highest_throughput['name']} on PG{highest_throughput['pg_version']} ({highest_throughput['throughput']:,.0f} ops/sec)")
